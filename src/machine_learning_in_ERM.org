...
[feature]+list([i[0]]+ [round(j,4) for j in i[1].values()] for i in result.items())
#+end_src

#+RESULTS:
| model             | precision | recall |     f1 | \(R^2\) |
| random            |    0.2275 | 0.1251 | 0.1521 | -0.0012 |
| bp neural network |     0.299 | 0.3547 | 0.2716 | -0.0612 |
| CNN               |    0.3944 | 0.4261 | 0.4039 |  0.3987 |
#+begin_src jupyter-python
import numpy as np
